...
def test():
    ...
    for x in xrange(SELECTS_ITERATIONS):
        cursor.execute ("SELECT `tests_user`.`id`, `tests_user`.`name`, `tests_user`.`email`, `tests_user`.`age`, `tests_user`.`power_level`, `tests_user`.`info` FROM `tests_user` LIMIT %s", SUB_SELECT_ITERATIONS)
